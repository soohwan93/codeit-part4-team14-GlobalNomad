...
const NotificationPopup = ({ setState, notifications, setNotifications, buttonPosition }: NotificationPopupProps) => {
  ...
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const cursorId = notifications.length > 0 ? notifications[notifications.length - 1].id : 0;
  ...
  useEffect(() => {
    const adjustPosition = () => {
      ...
      const popupHeight = notificationsRef.current ? notificationsRef.current.offsetHeight : 0;
  ...
  return (
    <div
      ...
      className="absolute z-[50] w-full px-5 py-6 rounded-[10px] border border-[#CBC9CF] bg-[#CED8D5] text-[#1b1b1b] md:w-[368px]"
      style={{
